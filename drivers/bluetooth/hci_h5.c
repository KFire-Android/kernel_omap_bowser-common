		...
		int len, int pkt_type)
{
	...
	if (h5extn && chan == 1) {
		...
		if (hci_opcode_ogf(__le16_to_cpu(opcode)) == 0x3f) {
			...
			desc = *(data + HCI_COMMAND_HDR_SIZE);
			if ((desc & 0xf0) == 0xc0) {
...
#ifdef DEBUG
#define H5_LAST_MAX 128
static unsigned char lastBuffer[H5_LAST_MAX];
static int lastLen;
#endif
...
static int h5_recv(struct hci_uart *hu, void *data, int count)
{
	...
	while (count) {
...
#ifdef DEBUG
				BT_DBG("Short H5 %02x, data %p, count orig %d remain %d, h5 rx_count %d state %d\n",
					*ptr, data, orig_count, count, (int) h5->rx_count, h5->rx_state);

				//print_hex_dump_bytes("h5_recv ", DUMP_PREFIX_NONE, data, orig_count);

				printk("last: %d", lastLen);
				for (i = 0; (i < lastLen) && (i < H5_LAST_MAX); i++)
				{
					printk(" %02X", lastBuffer[i]);
				}
				printk("\n");

				printk("current: ");
				for (i = 0; (i < orig_count) && (i < 32); i++)
				{
					printk(" %02X", pData[i]);
				}
				printk("\n");
		...
		case H5_W4_PKT_DELIMITER:
			...
			case 0x11:
			case 0x13:
				// Just in case the lower layer didn't filter out the XON/XOFF characters				// hci_uart_tx_wakeup(hu);
				break;
		...
		case H5_W4_PKT_START:
			...
			case 0x11:
			case 0x13:
				// Just in case the lower layer didn't filter out the XON/XOFF characters
				// hci_uart_tx_wakeup(hu);
				ptr++; count--;
				break;
...
#ifdef DEBUG
	for (i = 0; (i < orig_count) && (i < H5_LAST_MAX); i++)
	{
		lastLen = orig_count;
		lastBuffer[i] = pData[i];
	}
