...
EXPORT_SYMBOL(smb347_reset_mode);
...
#ifdef SUMMIT_SMB347_DEBUG
	dev_info(priv->dev, "Change input current limit to %d\n",
...
#ifdef SUMMIT_SMB347_DEBUG
	if (force) {
		dev_info(priv->dev, "Disable AICL and force HC mode.\n");
	} else {
		dev_info(priv->dev, "Enable AICL and redo APSD.\n");
	}
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if (priv->i2c_client->irq != -1) {
		gpio = irq_to_gpio(priv->i2c_client->irq);
		if (gpio_is_valid(gpio)) {
			gpio_request(gpio, "smb347-irq");
			gpio_direction_input(gpio);
		} else {
			dev_err(priv->dev, "Invalid gpio %d\n", gpio);
			priv->i2c_client->irq = -1;
		}
	}
