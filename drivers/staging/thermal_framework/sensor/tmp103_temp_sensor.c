...
static const unsigned short normal_i2c[] = { 0x70, 0x71, 0x72, 
					0x73, 0x74, 0x75, 0x76, 0x77, I2C_CLIENT_END };
		...
		struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	thermal_sensor_set_temp(tmp103->therm_fw);
	/* Send a kobj_change */
...
static DEVICE_ATTR(temp1_input, S_IRUGO, tmp103_temp_sensor_read_temp,
			  NULL);
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (tmp103->therm_fw) {
		...
		thermal_sensor_dev_register(tmp103->therm_fw);
	} else {
		ret = -ENOMEM;
...
//#ifdef CONFIG_PM
static int tmp103_temp_sensor_suspend(struct i2c_client *client,
			pm_message_t mesg)
{
...
static int tmp103_detect(struct i2c_client *new_client,
		       struct i2c_board_info *info)
{
	strlcpy(info->type, "tmp103_temp_sensor", I2C_NAME_SIZE);
