/*
 * drivers/cpufreq/cpufreq_perfactive.c
 *
...
struct cpufreq_perfactive_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_perfactive_cpuinfo, cpuinfo);
...
/*
 * Period, during what we keep MPU at boost's frequency
 * after calling cpufreq_perfactive_do_boost function
 */
static unsigned int boost_timeout = 5000;
static struct mutex boost_lock;
/*
 * Keep low frequncy limit during the boost period
 */
static unsigned int low_freq_limit_save;
/*
 * Flag, needed to check is MPU is boosted or not
 */
static unsigned int boost_is_active = 0;

static struct timer_list boost_timer;

static int cpufreq_governor_perfactive(struct cpufreq_policy *policy,
		unsigned int event);
static int show_video_hint(char *buf);
static void store_video_hint(struct cpufreq_policy *policy, unsigned int hint);
static int show_panel_hint(char *buf);
static void store_panel_hint(struct cpufreq_policy *policy, unsigned int hint);
static int cpufreq_perfactive_do_boost(struct cpufreq_policy *policy);

static unsigned int perfactive_boost_timeout_get(struct cpufreq_policy *policy) {

	return boost_timeout;
}
void perfactive_boost_timeout_store(struct cpufreq_policy *policy, unsigned int timeout) {

	boost_timeout = timeout;
}

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PERFACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_perfactive = {
	.name = "perfactive",
	.governor = cpufreq_governor_perfactive,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
	.boost_cpu_freq = cpufreq_perfactive_do_boost,
	.show_video_hint = show_video_hint,
	.store_video_hint = store_video_hint,
	.show_panel_hint = show_panel_hint,
	.store_panel_hint = store_panel_hint,
	.show_boost_timeout = perfactive_boost_timeout_get,
	.store_boost_timeout = perfactive_boost_timeout_store,
};

static void cpufreq_perfactive_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_perfactive_cpuinfo *pcpu =
	...
	if (cpufreq_frequency_table_target(pcpu->policy, pcpu->freq_table,
		...
		if (!warn_once) {
			warn_once = true;
			pr_warn("timer %d: cpufreq_frequency_table_target error"
				"for new_freq %d\n", (int) data, new_freq);
			pr_warn("current freq table: ");
			for (i = 0; pcpu->freq_table[i].frequency != CPUFREQ_TABLE_END; i++)
					pr_warn("%d ", pcpu->freq_table[i].frequency);
			pr_warn("\n");
		}
		goto rearm;
	}
...
static void cpufreq_perfactive_idle_start(void)
{
	struct cpufreq_perfactive_cpuinfo *pcpu =
...
static void cpufreq_perfactive_idle_end(void)
{
	struct cpufreq_perfactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
...
static int cpufreq_perfactive_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_perfactive_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_perfactive_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
...
static void cpufreq_perfactive_boost(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
...
static struct attribute *perfactive_attributes[] = {
...
static struct attribute_group perfactive_attr_group = {
	.attrs = perfactive_attributes,
	.name = "perfactive",
};

static int cpufreq_perfactive_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_perfactive_idle_start();
		break;
	case IDLE_END:
		cpufreq_perfactive_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_perfactive_idle_nb = {
	.notifier_call = cpufreq_perfactive_idle_notifier,
};

enum perfactive_mode {
	PERFORMANCE,
	INTERACTIVE,
};

static void switch_mode(enum perfactive_mode mode,
		struct cpufreq_policy *policy)
{
	unsigned int i;
	struct cpufreq_perfactive_cpuinfo *this_cpu_info;

	if (mode == PERFORMANCE) {
		/* switch to performance mode */
		for_each_cpu(i, policy->cpus) {
			this_cpu_info = &per_cpu(cpuinfo, i);
			this_cpu_info->governor_enabled = 0;
			smp_wmb();
			del_timer_sync(&this_cpu_info->cpu_timer);
			this_cpu_info->idle_exit_time = 0;

		}
		idle_notifier_unregister(&cpufreq_perfactive_idle_nb);
		...
		for_each_cpu(i, policy->cpus) {
			this_cpu_info = &per_cpu(cpuinfo, i);
			this_cpu_info->governor_enabled = 1;
			smp_wmb();
		}
		idle_notifier_register(&cpufreq_perfactive_idle_nb);
	}
}
...
static void store_video_hint(struct cpufreq_policy *policy, unsigned int hint)
{
	...
	if (old_mode != new_mode)
		switch_mode(new_mode ? INTERACTIVE : PERFORMANCE, policy);
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		/*
		 * Hints for switching between performance or interactive mode
		 */
		video_playback_on = 0;
		panel_on = 1;
		...
		pr_info("%s for_each_cpu: %d\n", __func__, j);

		if (!hispeed_freq)
			hispeed_freq = policy->max;

		/*
		 * Set cpufreq to max by default
		 * We will switch to interactive mode upon receiving hint
		 */
		__cpufreq_driver_target(policy, policy->max,
				CPUFREQ_RELATION_H);
...
static void cpufreq_perfactive_boost_timer(unsigned long data)
{
...
static int __init cpufreq_perfactive_init(void)
{
	unsigned int i;
	struct cpufreq_perfactive_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_perfactive_timer;
		pcpu->cpu_timer.data = i;
		pcpu->cpu_boost_value = DEFAULT_BOOST;
	}

	pr_info("%s possible_cpus: %d\n", __func__, i);

	init_timer(&boost_timer);
	boost_timer.function = cpufreq_perfactive_boost_timer;
	up_task = kthread_create(cpufreq_perfactive_up_task, NULL,
				 "kperfactiveup");
	if (IS_ERR(up_task))
	...
	INIT_WORK(&freq_scale_down_work,
		  cpufreq_perfactive_freq_down);

	INIT_WORK(&boost_work,
		  cpufreq_perfactive_boost);
	...
	return cpufreq_register_governor(&cpufreq_gov_perfactive);
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PERFACTIVE
fs_initcall(cpufreq_perfactive_init);
#else
module_init(cpufreq_perfactive_init);
#endif

static void __exit cpufreq_perfactive_exit(void)
{
	del_timer_sync(&boost_timer);
	cpufreq_unregister_governor(&cpufreq_gov_perfactive);
	kthread_stop(up_task);
	put_task_struct(up_task);
	destroy_workqueue(down_wq);
	destroy_workqueue(boost_wq);
	mutex_destroy(&hint_lock);
}

module_exit(cpufreq_perfactive_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_perfactive' - A cpufreq governor for "
