...
static void dsscomp_mgr_delayed_cb(struct work_struct *work)
{
	...
	if (status == DSS_COMPLETION_PROGRAMMED) {
		...
		TRAPZ_DESCRIBE(TRAPZ_KERN_DISP_DSS, DssCompDelayedCBProgrammed, 
                        "dsscomp_mgr_delayed_cb: DSSCOMP Delay Cb status: PROGRAMMED - Propagate configuration info to DISPC");
		...
		TRAPZ_DESCRIBE(TRAPZ_KERN_DISP_DSS, DssCompDelayedCBDisplayed,
			"dsscomp_mgr_delayed_cb: DSSCOMP Delay Cb status: DISPLAYED - Buffer has been displayed");
		...
		TRAPZ_DESCRIBE(TRAPZ_KERN_DISP_DSS, DssCompDelayedCBReleased,
			"dsscomp_mgr_delayed_cb: DSSCOMP Delay Cb status: RELEASED - Buffer has been displayed and the last info is overwritten by new configuration");
...
skip_ovl_set:
		...
		TRAPZ_DESCRIBE(TRAPZ_KERN_DISP_DSS, DssCompApplyLogAddr,
			"dsscomp_apply: Log gralloc and phys addresses");
		TRAPZ_LOG_PRINTF(TRAPZ_LOG_DEBUG, TRAPZ_CAT_KERNEL, TRAPZ_KERN_DISP_DSS, DssCompApplyLogAddr, "gralloc addr %d paddr %d",  comp->extra_cb_data, oi->ba);
	}
	...
	TRAPZ_DESCRIBE(TRAPZ_KERN_DISP_DSS, DssCompApply,
		"dsscomp_apply: DSSCOMP apply composition");
