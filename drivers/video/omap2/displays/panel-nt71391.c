...
void nt71391_read_vendor_id(struct omap_dss_device *dssdev)
{
	...
	r = dsi_vc_gen_write_nosync(dssdev, d2d->channel0, buf, 2);
	if(r < 0)
	{
		printk("Error in sending lock cmd\n");
		goto err;
	...
	reg = 0x8D;
	r = dsi_vc_gen_read_2(dssdev, d2d->channel0, reg, &vendor_id[0], 2);
	if(r < 0)
	{
	...
	reg = 0x8E;
	r = dsi_vc_gen_read_2(dssdev, d2d->channel0, reg, &vendor_id[1], 2);
	if(r < 0)
	{
		printk("Error in dsi read\n");
		goto err;
	...
	reg = 0x8F;
	r = dsi_vc_gen_read_2(dssdev, d2d->channel0, reg, &vendor_id[2], 2);
	if(r < 0)
	{
		printk("Error in dsi read\n");
		goto err;
	...
	buf[0] = 0;
	buf[1] = 0;
	r = dsi_vc_gen_write_nosync(dssdev, d2d->channel0, buf, 1);
	if(r < 0)
	{
		printk("Error in sending unlock cmd\n");
		goto err;
...
static int nt71391_suspend(struct omap_dss_device *dssdev)
{
	...
	/*Wait for Backlight to turn OFF before turning OFF the LCD */
	//bl_sleep = gpio_get_value(GPIO_BACKLIGHT_EN_O2M);
