...
/*
 * FLAG : to prevent all IO to MPU once it shuts down
 */
extern __u8 mpu_device_is_shutdown;
				    ...
				    unsigned int reset, unsigned long sensors)
{
	...
	if (pwr_mgmt[0] != pwr_mgmt_prev[0] && !(pwr_mgmt[0] & BIT_SLEEP)) { msleep(50); }
